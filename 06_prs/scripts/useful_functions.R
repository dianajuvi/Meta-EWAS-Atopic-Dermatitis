...
{
	if (!model_letter %in% c("a" ,"b", "c", "d")) stop("model must be either a, b, c or d")
	...
	covariates <- list(a = c("sex", sv_covs), 
					   ...
					   d = c("sex", sv_covs, "mothers_social_class", "sustained", "mothers_age_years", "gestage", 
					   		 "Bcell", "CD4T", "CD8T", "Gran", "Mono", "NK", "nRBC", "prs_z")
					   )
